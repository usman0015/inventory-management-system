...
<span class="sep">/</span>
<span class="current">Create Order</span>
@endsection

@section('content')
<div class="form-page">
    <div class="page-header">
        <h4><i class="bi bi-plus-circle me-2 text-success"></i>Create New Order</h4>
    ...
    <div class="app-card form-card">
        <div class="app-card-header">
            <h6 class="mb-0"><i class="bi bi-cart-plus me-2 text-primary"></i>Order Information</h6>
        </div>
        <div class="app-card-body">
            <form action="{{ route('orders.store') }}" method="POST" data-loading>
                @csrf
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <div class="input-icon">
                                ...
                                <input type="text" id="customer_name" name="customer_name" value="{{ old('customer_name') }}" class="form-control {{ $errors->has('customer_name') ? 'is-invalid' : '' }}" placeholder="e.g. John Smith" required>
                            </div>
                            ...
                            <div class="input-icon">
                                ...
                                <select name="item_id" id="item_id" class="form-select {{ $errors->has('item_id') ? 'is-invalid' : '' }}" required>
                                    ...
                                    @foreach($items as $item)
                                        <option value="{{ $item->id }}" {{ old('item_id') == $item->id ? 'selected' : '' }}>{{ $item->name }} — ${{ number_format($item->price, 2) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-icon">
                                ...
                                <input type="number" name="quantity" id="quantity" min="1" value="{{ old('quantity', 1) }}" class="form-control {{ $errors->has('quantity') ? 'is-invalid' : '' }}" placeholder="e.g. 3" required>
                            </div>
